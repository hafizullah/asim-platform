...
class SimSvgLogo extends StatelessWidget {
  ...
  const SimSvgLogo({
    ...
    this.width,
    this.height,
    this.variant = SimSvgLogoVariant.brand,
    this.fit = BoxFit.contain,
    this.color,
  });

  /// Small logo for app bars (uses icon variant)
  const SimSvgLogo.small({
    super.key,
    this.color,
  }) : width = 120,
       height = 36,
       variant = SimSvgLogoVariant.icon,
       fit = BoxFit.contain;

  /// Medium logo for headers (uses brand variant)
  const SimSvgLogo.medium({
    super.key,
    this.color,
  }) : width = 200,
       height = 60,
       variant = SimSvgLogoVariant.brand,
       fit = BoxFit.contain;

  /// Large logo for hero sections (uses brand variant)
  const SimSvgLogo.large({
    super.key,
    this.color,
  }) : width = 400,
       height = 120,
  ...
  String _getAssetPath() {
    switch (variant) {
      case SimSvgLogoVariant.brand:
        return LogoAssets.brandLogo;
      case SimSvgLogoVariant.icon:
        return LogoAssets.appIcon;
      case SimSvgLogoVariant.full:
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (variant == SimSvgLogoVariant.brand || variant == SimSvgLogoVariant.full) ...[
...
enum SimSvgLogoVariant {
  /// Full brand logo with text and taglines (logo.svg)
  full,
  
  /// Horizontal brand logo with sim.af text (brand_logo.svg)
  brand,
